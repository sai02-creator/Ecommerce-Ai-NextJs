...
export const orderType = defineType({
  ...
  fields: [
        ...
        defineArrayMember({
          ...
          preview: {
            ...
            prepare({ title, quantity, price, media }) {
              return {
                title: title ?? "Product",
                subtitle: `Qty: ${quantity} • A$${price}`, // Changed to AUD
                media,
              };
            },
          },
    ...
    defineField({
      ...
      group: "details",
      readOnly: true,
      description: "Total order amount in AUD", // Changed to AUD
    }),
    defineField({
      name: "status",
      type: "string",
  ...
  preview: {
    ...
    prepare({ orderNumber, email, total, status }) {
      return {
        ...
        subtitle: `${email ?? "No email"} • A$${total ?? 0} • ${status ?? "paid"}`, // Changed to AUD
      };
    },
  },
  orderings: [
